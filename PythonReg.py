...
data = pd.read_csv('/Users/wiseer85/Desktop/gapminder.csv', low_memory=False)
...
####################################################################################
#ordered linear regression (OLS) analysis
...
#Q-Q plot to test for normality by plotting residuals of model from data
...
print(chk4)
pandas.crosstab(sub1['S3AQ8B11'], sub1['S3AQ8B12'])
...
pandas.crosstab(sub1['TAB12MDX'], sub1['NICOTINEDEP'])
...
##############################################################################
#multivariate models with no. cigarettes and centered IVs
...
##############################################################################
#center continuous explanatory variables for regression analysis
sub1['numbercigsmoked_c'] = (sub1['numbercigsmoked'] - sub1['numbercigsmoked'].mean())
print (sub1['numbercigsmoked_c'].mean()) 
sub1['age_c']=(sub1['AGE'] - sub1['AGE'].mean())
print (sub1['age_c'].mean()) 
...
# odd ratios (probability for one group vs another) with 95% confidence intervals
params = lreg1.params
conf = lreg1.conf_int()
conf['OR'] = params
